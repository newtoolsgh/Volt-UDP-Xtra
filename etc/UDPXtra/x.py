...
if action == "manage":
    ...
    if sys.argv[2] == "add":
        if len(sys.argv) < 6:
            print("Requerid user pass and expire time format 2023-12-31")
            sys.exit(1)

        os.system("docker exec -it udpr adduser " +
                  sys.argv[3]+" --disabled-password")
        os.system("docker exec -it udpr chage -E "+sys.argv[5]+" "+sys.argv[3])
        cmdChangePassword = "docker exec -it udpr sh /root/useradd.sh \"" + \
...
if action == "detail":
    ...
    try:
        result = subprocess.run(['finger', username],
